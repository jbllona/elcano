...
enum class ParseStateError : int8_t {
  ...
  passthru    = 7, //!< An uncaptured message has been passed throu
  no_space    = 8,
  noise_in_data = 9,
  no_comma = 10
...
struct ParseState {
  ...
  ParseStateError checkDrive(void);
  ParseStateError checkGoal(void);
  ParseStateError checkSeg(void);
  ParseStateError checkSensor(void);
  
  bool is_expectedChar(char c, char sample);
  bool is_Number(char c);
  int char2Int(char c);
  int intBuff2Int(int n);
  int power(int base, int iexp);
